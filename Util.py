...
def OneHot(num, label_size=2):#changed********************************************************************************************************** label_size=7=> label_size=2
...
def preprocess_nltk(text, stoplist):
    ...
    import nltk.stem
    s = nltk.stem.SnowballStemmer('english')  #파라미터는 선택 언어
...
def readFileList(DatasetName):
    filelist = open("DataSet\\{}\\FileNumberList.txt".format(DatasetName), encoding="utf-8").read().split("\n")
    if len(filelist[-1])==0:
        filelist = filelist[:-1]
    intfileList = []
    for i in filelist:

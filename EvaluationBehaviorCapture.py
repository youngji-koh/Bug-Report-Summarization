...
# Normalzation (0~1사이 값)
def min_max_normalize(lst, sentenceWeight):
    normalized = []

    mmin = min(lst)
    mmax = max(lst)
    if mmax - mmin == 0:
        normalized = [1.0] * len(lst)
    else:
        for value in lst:
            normalized_num = 1 + ((value - mmin) / (mmax - mmin) * sentenceWeight)
            normalized.append(normalized_num)
    return normalized



# Z-Score normalization (Standardization 표준화 : 평균 0, 표준편차 1)
...
def EvaluationBehaviorCap(dataList, sheetName, domainwordThreshold, alpha, beta, gamma, sentenceWeight):
    sheetNum = len(dataList)

    for i in range(sheetNum):
        #print("Processing :", sheetName[i])
        SenList, lemSenList, senNumberList, authorList, senInvolvecd, senVecList, senOPscore, senTRscore, senTPscore, senRank, tfidfWordList, tfidfScoreList, ignoreList = SigDataInput(dataList, i)
        ...
        # standardization
        # FscoreList = z_score_normalize(FscoreList)
        # senTRscore = z_score_normalize(senTRscore)

        # scailing (normalization)
        FscoreList = min_max_normalize(FscoreList, sentenceWeight)
        senTRscore = min_max_normalize(senTRscore, sentenceWeight)
        senTPscore = min_max_normalize(senTPscore, sentenceWeight)

        # robust scaler
        # robustScaler = RobustScaler()
        # FscoreList_2d = np.transpose([FscoreList])
        # senTRscore_2d = np.transpose([senTRscore])
        #
        # transformer_F = robustScaler.fit(FscoreList_2d)
        # transformer_TR = robustScaler.fit(senTRscore_2d)
        #
        # FscoreList = transformer_F.transform(FscoreList_2d)
        # senTRscore = transformer_TR.transform(senTRscore_2d)
        #
        #
        #
        # print("Fscore############################")
        ...
        believabiltyList = FscoreList[:]

        for j in range(len(FscoreList)):
            #print("setence believability: " , FscoreList[j])
            FscoreList[j] = believabiltyList[j] * alpha + senTRscore[j] * beta + senTPscore[j] * gamma

            # sentTRscore or senRank 이용
            #FscoreList[j] = senTRscore[j] * beta + senTPscore[j] * gamma

            #FscoreList[j] = (FscoreList[j]*alpha + (senTRscore[j])*(1-alpha) * 100) * 100000
            #FscoreList[j] = FscoreList[j] * alpha + (senRank[j]) * (1 - alpha)
            #FscoreList[j] = 1
        #print("min:", min(senTRscore))
        #print("max:", max(senTRscore))
        #print(FscoreList)

        dataList[i]["Believability"] = believabiltyList
...
def directlycite(sentencelist, sentenceInvolved):
    ...
    for i in range(commentnum):
        for sennumber in sentenceInvolved[i]:
            ...
            # '>' 대신 '&amp;gt;' 사용
            if ('>' in str(sentencelist[int(sennumber)])):
...
def senCheckForKeyWordAscreSenEval(sentencelist, sentenceInvolved, EvaluatorSentenceNumberList):
    ...
    for comNumber in range(ComNum):
        for senNumber in sentenceInvolved[comNumber]:
            ...
            if ">" in str(sentencelist[senNumber]) or "?" in str(sentencelist[senNumber]):
                skipSentenceList[senNumber] = 1
    ...
    for i in range(UnionNum):
        ...
        for j in range(imptSenNum):
            ...
            for z in range(quotedSenNumber):
                if (quotedSen in str(sentencelist[z])):
                    flag = 1
...
def CommentNumberCite(sentencelist, evaluationTimeList, sentenceInvolved, domainwordlist, evaluationList, EvaluatorSentenceNumberList, CombinedQuotedSentenceList, ignoreList):
    ...
    for i in range(commentnum):
        for sennumber in sentenceInvolved[i]:
            ContainedCNCite = CheckCommentNumber(str(sentencelist[int(sennumber)]))
...
def CheckShareDomainWordBetweenSen(sena, senb, sentencelist, domainwordlist):
    ...
    senalist = str(sentencelist[sena]).split()
    senblist = str(sentencelist[senb]).split()
    for i in domainwordlist:
...
def EvaluationBehaviorCapture(datasetName, alpha, beta, gamma, sentenceWeight):
    sheetNameList, dataList = DataInput(datasetName)
    # 원래 코드에는 0.1로 되어있었음 (논문에는 0.08)
    dataList = EvaluationBehaviorCap(dataList, sheetNameList, 0.08, alpha, beta, gamma, sentenceWeight)
    wirteDataList(dataList, sheetNameList, datasetName)



if __name__ == "__main__":
    EvaluationBehaviorCapture("SDS", 0)